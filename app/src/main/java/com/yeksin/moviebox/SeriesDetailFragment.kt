...
class SeriesDetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        seriesId = arguments?.getInt("series_id") ?: return
        seriesId?.let {
            fetchMovieDetail(it)
            fetchCast(it)
        }
    ...
    private fun fetchMovieDetail(seriesId: Int) {
        val request = ApiService.getRequestBuilder("tv/$seriesId?language=en-US&append_to_response=credits").build()

        ApiService.getClient().newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                e.printStackTrace()
                requireActivity().runOnUiThread {
                    Toast.makeText(requireContext(), "Error fetching series details", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                response.use {
                    ...
                    if (responseData != null) {
                        try {
                            val gson = Gson()
                            val seriesDetail = gson.fromJson(responseData, SeriesDetail::class.java)
                            requireActivity().runOnUiThread {
                                updateUI(seriesDetail)
                            }
                        } catch (e: Exception) {
                            e.printStackTrace()
                            requireActivity().runOnUiThread {
                                Toast.makeText(requireContext(), "Error parsing series details", Toast.LENGTH_SHORT).show()
                            }
                        }
                    }
                }
    ...
    private fun fetchCast(seriesId: Int) {
        ...
        castRepository.seriesId = seriesId
        castRepository.fetchSeriesCast { castResponse ->
            castResponse?.let {
                requireActivity().runOnUiThread {
                    updateCastUI(it)
                }
    ...
    private fun updateUI(seriesDetail: SeriesDetail) {
        view?.let { rootView ->
            ...
            val fullOverviewText = seriesDetail.overview
            ...
            titleTextView.text = seriesDetail.original_name
            releaseDateTextView.text = seriesDetail.first_air_date.split("-")[0]
            genresTextView.text = seriesDetail.genres.joinToString(", ") {it.name}
            if (seriesDetail.tagline.isNullOrEmpty()) {taglineTextView.visibility = View.GONE} else {taglineTextView.text = seriesDetail.tagline}
            titleCast.text = "Cast"

            // Load images
            Picasso.get().load("https://image.tmdb.org/t/p/w500${seriesDetail.poster_path}").into(posterImageView)
            Picasso.get().load("https://image.tmdb.org/t/p/w1280${seriesDetail.backdrop_path}").into(backdropImageView)
        }
    }
